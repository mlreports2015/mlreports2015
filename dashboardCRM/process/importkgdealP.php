...
$data = explode("\n",$csvString);
	...
	for($n = 1; $n < count($rowfields)-1; $n++){
		...
		foreach($rowfields[$n] as $field){
		  ...
		  if($rowfields[0][$fieldCount]=='Deal ID')
		  {
				 $deal['deal_Id'] = $field; 
				 $order['deal_id']= $field;
				 
		  }else if($rowfields[0][$fieldCount]=='Transaction ID'){
			  
			     $order['order_id'] = $field;
		  		 $deliver['order_id']= $field;
				  
		  }else if($rowfields[0][$fieldCount]=='Purchase Date'){
			      $order['dated']=$field;
		  }else if($rowfields[0][$fieldCount]=='Options'){
			      $deal['dealName']=$field;
		  }else if($rowfields[0][$fieldCount]=='Voucher Code'){
			       $order['vouch_Code']=$field;
		  }else if($rowfields[0][$fieldCount]=='Customer Name'){
				   $person['clientName']=$field;
		  }else if($rowfields[0][$fieldCount]=='Surname'){
			  	   $person['clientName']=$person['clientName']." ".$field;
		  }else if($rowfields[0][$fieldCount]=='Address 1'){
				    $person['address1']=$field;
		  }else if($rowfields[0][$fieldCount]=='shipment_address_street_2'){
			  		$person['address2']=$field;
		  }else if($rowfields[0][$fieldCount]=='Address 2'){
			  	    $person['city']=$field;
		  }else if($rowfields[0][$fieldCount]=='Zipcode'){
			  		$person['pCode']=$field;
