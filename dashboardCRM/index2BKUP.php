...
$apptsStatement = "SELECT case_id, client_name, agency_ref, FROM_UNIXTIME( `slot-time` )as app_date, `clinic-id`, clinic_id FROM `case` JOIN `clinic_tmslot` ON `case-id` = case_id JOIN `clinic_date` ON `clinic-id` = diary_id WHERE usr_id ='".$_SESSION['usr_id']."'";
...
<div class="navbar navbar-default" style="width:100%;background-color:rgba(8,82,126,0.8);margin:0px;height:30px;">
  <div class="container-fluid">
    ...
    <div class="breadcrumb" style="text-align:right;font-family:'Times New Roman', Times, serif;font-size:larger;font-style:normal;background-color:#a616c5;"><a href="companySave.php" ><?php echo $_SESSION['usr'] ;?></a> currently logged in</div>
   ...
   <h3 align="left">MONTHLY STATS</h3>
    ...
    <div style="float:left;width:30%">
        <h2>Appointments: <span class="badge" style="width:80px;height:40px;font-size:30px;font-weight:bold;" ><?php echo mysql_num_rows($apptsStatementRes); ?></span></h2>
    </div>
        <div style="float:left;width:30%;">
...
<div align='center' style="width:90%">
    ...
    <div style="width:100%;text-align:left;font-size:24px;padding-top:10px;">10 LATEST APPOINTMENTS <span id="moreapps" name="moreapps" title="more" style="cursor: pointer">+</span></div>

  <div class="alert alert-info" id="recentapps" name="recentapps" style="padding:0px;margin:0px;border:none;padding-bottom:14px;" >
...
<table width="95%" cellspacing='5px' cellpadding='5px'>
<tr><th>Agency Ref.<th>Solicitor Ref.</th><th>App Date</th><th>Location</th></tr>
    ...
    // if($jsndecde2->success){
	  ...
	  // var_dump($jsndecde2->appointment[0]);
	   ...
	   //foreach($jsndecde2->appointment as $value){
			...
			echo "<tr>";
			echo "<td><a href='#'  >".$apptrecs['agency_ref']."</td>";
			echo "<td>".$value->sRef."</td>";
			echo "<td>".$apptrecs['app_date']."</td>";
			echo "<td>".$apptrecs['clinic_id']."</td>";
			echo "</tr>";
