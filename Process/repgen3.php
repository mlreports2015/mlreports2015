...
if($_POST['reptyp']=='child'){

		$accompnw="select * from accomp where cid = '".$id."'";
		//echo $accomp;
		$accompRes = mysql_query($accompnw);
		$accompPrint = mysql_fetch_array($accompRes);
		
		$str22=str_replace("|Lnm|",trim($accompPrint['afn']).' '.trim($accompPrint['aln']),$str22);
...
// on examination
if($eon !="Was not examined."){
$strEF=str_replace("|eon|",$eon,$strEF);
}
$strEF=str_replace("|eou|",$eou,$strEF);

if(strpos("Was not examined.")===false){
$strEF=str_replace("|eob|",$eob,$strEF);
}else{
$strEF=str_replace("|eob|","",$strEF);
}
if(strpos("Was not examined")===false){
 $strEF=str_replace("|eot|",$eot,$strEF);
}else{
 $strEF=str_replace("|eot|","",$strEF);
}
if(strpos($eol,"Was not examined")===false){
   $strEF=str_replace("|eol|",$eol,$strEF);
}else{
$strEF=str_replace("|eol|","",$strEF);
}
...
$ttlPg->writeText('<br><br>', new Font(13, $fF, $fFC), new parFormat('center'));

$table = &$ttlPg->addTable();
$table->addRows(1);
$table->addColumnsList(array(8,8));
$cell = &$table->getCell(1, 1);
$img=&$cell->addImage("Logos/bond-solon-logo.jpg",$null, 4, 0, new parFormat('center'));
$img=&$cell->addImage("Logos/ewi-logo.jpg",$null, 3, 2, new parFormat('right'));
//creating first page
...
//echo $ade.'<br>';
$ade=ageCalc3($claimant['cdb'], $claimant['cda']);
//echo $ade2;
...
$cell2->writeText($ade, new Font(12, $fF, $fFC), $par0LeftBor);
...
$ident2 = substr($ident,(strpos($ident,'Photographic')+13));



if (strpos($ident,'Photographic')>0)
{
	$cell2->writeText('<br><br><br> Q ', new Font(12, 'Wingdings 2', $fFC), $null);
...
else
{
	...
	$cell2->writeText('Yes ', new Font(12, $fF, $fFC), $null);
	$cell2->writeText(' Q', new Font(12, 'Wingdings 2', $fFC), $null);
	$cell2->writeText('No', new Font(12, $fF, $fFC), $null);
}
// 
...
$cell2->writeText("<strong>    If Yes, what type of photo ID was Checked<br/>       Identity No: </strong>", new Font(12, $fF, $fFC), $par0Left);
$cell2=&$table2->getCell(7, 2);

if(strpos($ident, 'Photographic')!=false){
$cell2->writeText($ident2, new Font(12, $fF, $fFC), $par0LeftBor);
}else{
...
if($_POST['reptyp']=='child'){
		...
		$cell2->writeText("<strong>Accompanied By </strong>", new Font(12, $fF, $fFC), $par0Left);
		$cell2=&$table2->getCell(9, 2);
		$cell2->writeText($accomp['aby'], new Font(12, $fF, $fFC), $par0LeftBor);	
...
$cell2=&$table2->getCell(16,1);
$cell2->writeText("<strong>1.10 Claimant Measurements</strong>", new Font(12, $fF, $fFC), $par0Left);
$cell2=&$table2->getCell(16,2);
$cell2->writeText("", new Font(12, $fF, $fFC), $par0LeftBor);
$cell2=&$table2->getCell(17,1);
$cell2->writeText("<strong>       Height:<br/>      Weight:</strong>", new Font(12, $fF, $fFC), $par0Left);
$cell2=&$table2->getCell(17,2);
if($claimant['msreType']=='non-metric'){
$cheight = explode('.',$claimant['height']);
$cweight = explode('.',$claimant['weight']);
$cell2->writeText($cheight[0]."ft ".$cheight[1]."inches<br/>".$cweight[0]."stone ".$cweight[1]."llbs", new Font(12, $fF, $fFC), $par0LeftBor);	
}else{	
$cell2->writeText($claimant['height']."cm <br/>".$claimant['weight']."kg ", new Font(12, $fF, $fFC), $par0LeftBor);
}
...
$cell->writeText("<strong>Introduction</strong>", new Font(14, $fF, $fFC), $parLeftB);

$cell2 =&$table->getCell(2,1);

$cell2->writeText( "<br>The report was specifically required to contain details of any relevant pre-accident medical history, a summary on the nature and extent of the clinical negligence and injuries sustained due to alleged clinical negligence, any treatment subsequently received, a report on the present condition of the claimant and prognosis as to the likely timescale for the injuries to settle, claimant's capacity for work, any consequential financial loss, social and leisure activities.
I have been requested to address this report to the court and can confirm that it has been prepared with the requirements of the civil procedure rules.<br>", new Font(12, $fF, $fFC), $par0LeftBor);

$cell2->writeText("<strong>Qualifications and Experience</strong>", new Font(12,$fF, $fFC),$parLeftB);

$cell2->writeText('<br>'.$str2.'<br>', new Font(12,$fontR, $fRC), $par0LeftBor);

$cell2->writeText("<strong>Case Details</strong>", new Font(12,$fF, $fFC),$parLeftB);
...
if($_POST['reptyp']=='child'){
$cell->writeText("<br><strong>B(i) History Reported By Claimants Parent/Guardian</strong><br>", new Font(13, $fF, $fFC), new parFormat());
}else{
...
$cell->writeText("<strong>D(ii) On examination</strong><br>(based on the interview and observations at the time of examination)", new Font(13, $fF, $fFC), new parFormat());
...
$cell=&$table->getCell(2, 1);
$cell->writeText("<br><strong>Seatbelts</strong><br>", new Font(13, $fF, $fFC), new parFormat());
...
if ($rBelt['sBelt']==true)
{
	$cell2->writeText('<br> Q ', new Font(12, 'Wingdings 2', $fFC), $null);
	$cell2->writeText('Yes', new Font(12, $fF, $fFC), $null);
	$cell2->writeText('  o ', new Font(12, 'Wingdings', $fFC), $null);
	$cell2->writeText('No', new Font(12, $fF, $fFC), $null);
}
...
$cell=&$table->getCell(2, 1);
$cell->writeText("<br><strong>Future treatment and rehabilitation</strong>", new Font(13, $fF, $fFC), new parFormat());
...
$ppft=selprogRecomend('prog', $id);
//echo $ppft;


$ftr=explode('<br/><br/>', $ppft);
//print_r($ftr);
...
for ($i=1; $i<count($ftr); $i++)
{
	...
	$ftrF=explode('I recommend ', $ftr[$i]);
	  if(!empty($ftrF[1])){
	    $posx =strpos($ftrF[1], 'In my opinion');
		
	  $ftrFF=$ftrFF.'<bullet> I recommend '.substr($ftrF[1],0,$posx).'<br>';
      }else{
	  $ftrFF=$ftrFF.$ftrF[0].'<br>';
	  }
}
...
$cell2->writeText("<br><br>", new Font(12,$fF, $fRC), $parLeftBor);
$img=&$cell2->addImage("Logos/bond-solon-logo.jpg",$null, 4, 0, new parFormat('center'));
$img=&$cell2->addImage("Logos/ewi-logo.jpg",$null, 3, 2, new parFormat('right'));
...
}else if($_POST['repref']=='2'){
$cell2=&$table2->getCell(2, 1);
$cell2->writeText('<br> LVI REFERENCE HERE', new Font(12,$fontR, $fRC), $parBody);
}
...
function ageCalc2($dob, $doa)
{
  $datetime1 = new DateTime($dob);
  $datetime2 = new DateTime($doa);
  $interval = $datetime1->diff($datetime2);
  $years = $interval->format('%y'); 
  
  if($years <= 1 ){
    $years = $interval->format('%m').' months' ;
  }
  //$days = $interval->format('%R%a days');
  //echo (intval($days)/365).'<br/>';
  //echo $years;
...
function ageCalc3($dob, $doa){

$birthday_date = $dob;
$current_date = $doa; //today is 2011-10-04
$diff_in_mill_seconds = strtotime($current_date) - strtotime($birthday_date);
$age = floor($diff_in_mill_seconds / (365.2425 *60*60*24));
