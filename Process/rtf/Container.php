...
// ALL TEXT IN THE <CAPS></CAPS> TAG IS SET TO CAPITALS
			$text = preg_replace("/<CAPS[ ]*>(.*?)<\/CAPS[ ]*>/mi", "\\caps \\1\\caps0 ", $text);
			
// ALL TEXT IN THE <SCAPS></SCAPS> TAG IS SET TO SMALL CAPS
			$text = preg_replace("/<SCAPS[ ]*>(.*?)<\/SCAPS[ ]*>/mi", "\\scaps \\1\\scaps0", $text);


// PARAGRAPH FORMATTING

// EVERYTHING BETWEEN THE <PAR></PAR> TAG IS CONSIDERED A PARAGRAPH
			$text = preg_replace("/<PAR[ ]*>(.*?)<\/PAR[ ]*>/mi", "\\par ", $text);
			
// EVERYTHING BETWEEN THE <BPAR></BPAR> TAG IS CONSIDERED A BULLETED PARAGRAPH
			$text = preg_replace("/<BPAR[ ]*>(.*?)<\/BPAR[ ]*>/mi", "{\listtext\pard\plain\ltrpar 1.\tab}\pard\plain \ltrpar
\s15\ql \fi-360\li720\ri0\sa200\sl276\slmult1\widctlpar\wrapdefault\aspalpha\aspnum\faauto\ls1\adjustright\rin0\lin720\itap0\pararsid2305259\contextualspace \rtlch\fcs1 \af31507\afs22\alang1025 \ltrch\fcs0 
\f31506\fs22\lang2057\langfe1033\cgrid\langnp2057\langfenp1033 {\rtlch\fcs1 \af31507 \ltrch\fcs0 \insrsid2305259 \\1\\}", $text);

// EVERYTHING BETWEEN THE <cNPAR></cNPAR> GETS A LINE NUMBER WHICH CONTINUES FROM BEFORE
			$text = preg_replace("/<cNPAR[ ]*>(.*?)<\/cNPAR[ ]*>/mi", "\\scaps \\1\\scaps0", $text);
