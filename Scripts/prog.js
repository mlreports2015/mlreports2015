...
if(res!='resolved')
{
	...
	if(pr=='headache')
	{
		fin=fin+' Headache is a well known symptom of WAD and usually invariably resolves with other symptoms of WAD, it does not usually represent a signifacant underlying head injury.  The headache is usually due to entrapment of greater occipital nerve within the substance of muscles.';	
	}
	...
	else if (t=='cphysio')
	{
		...
		if (s.length>0)
			fin=fin+' for a further '+s+' sessions';
		fin=fin+'.';
	}
	else if (t=='physio')
	{
	...
	{
		fin=fin+'I recommend continuing dentist treatment';
	...
	{
		fin=fin+'I recommend continuing the psychologist treatment';
	...
	else if (t=='spec')
	{
		...
		fin=fin+'I recommend referral for an x-ray for further investigation.';
	else if(t=="MRI")
	    fin=fin+'I recommend referral for an MRI Scan for further investigation.';
	else if (t=='exer')
...
if(res!='resolved')
{
	...
	else if (c!='exacerbation of a pre-existing condition' && wr=='will resolve over')
	{
		...
		fin=fin+'The prognosis is dependent on the examination and assessment by the specialist.';
	else if (wr=='depends on the report')
		fin = fin + 'The prognosis is dependent on the results of the report.';
	else if (wr=='will not resolve')
	{
		fin=fin+'The '+pr+' will not resolve.';
	}
...
// 		alert(document.getElementById(v).value);
	...
	else if (document.getElementById(v).value=='xray')
	{
		
		createOption('frm',w,'depends on the report', 'depends on the report');
...
// check options for new item exixiting previously
...
function chkmnths(i)
{
	...
	var maxdtdiff = document.getElementById('dtdiff').value;
	var probId = document.getElementById('prob'+i).value;
	var valnum = 'resd'+i+'';
	var valresp = 'resp'+i+'';
	
	
	if(document.getElementById(valresp).value=='months'){
	
		if(maxdtdiff > parseInt(document.getElementById(valnum).value)){
			
	
			document.getElementById('wrnblock').style.display='block';
			document.getElementById('wrnblock').style.top=(document.body.scrollTop-100)+'px'
			document.getElementById('wrnsecblk').style.display='block';
			//alert(document.body.scrollTop);
			document.getElementById('idprob').value=probId;
				
		}
	
		
	}else if(document.getElementById(valresp).value=='years'){
		
		
		if((Math.round(maxdtdiff/12)) > parseInt(document.getElementById(valnum).value)){
...
function addTofinalComms(comms){

	document.getElementById('finalComms').textContent = comms;	
}


function addFinComms(){
	
	var comms = document.getElementById('finalComms').textContent;
	var ni = document.getElementById('pro');
	
	if(comms!=""){
	
      var newt = document.createElement('textarea');
	  var tIdName = 'finComms';
	   newt.setAttribute('cols',100);
	   newt.setAttribute('id',tIdName);
	   newt.setAttribute('name',tIdName);
	   newt.style.backgroundColor="#cfe2ee";
...
function offProgWarn(){
	
var dtdiffers = document.getElementById('dtdiff').value;
var cid = document.getElementById('cId').value;

document.getElementById('wrnblock').style.display='none';

alert('Incorrect prognosis period, please ensure that the prognosis period is over '+dtdiffers+' months');

document.location='summary.php?cid='+cid;

}

function progWarnCorr(){

	var prob = document.getElementById('idprob').value;
	var cid = document.getElementById('cId').value
	
	alert(prob+' has been resolved');
	
	document.getElementById('ifrmwarn').src='progExpireRes.php?cid='+cid+'&prob='+prob;
